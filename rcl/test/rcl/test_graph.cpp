 ...
 */
void expect_topics_types(const rcl_node_t *node,
...
public:
    ...
    void SetUp() override {
        ...
        *this->remote_context_ptr = rcl_get_zero_initialized_context();
        ret = rcl_node_init(remote_node_ptr, remote_node_name, "", this->remote_context_ptr, &node_options);
        ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
  ...
  void TearDown() override {
    ...
    EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
    RCUTILS_LOG_INFO_NAMED(ROS_PACKAGE_NAME, "Tearing down class");
    ret = rcl_context_fini(this->old_context_ptr);
    EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
    delete this->old_context_ptr;
  }
  ...
  void WaitForAllNodesAlive() {
     ...
     */
    void VerifySubsystemCount(const expected_node_state &&node_state,
            const expected_node_state &&remote_node_state) const {
        std::array<rcl_node_t *, 2> node_array{this->remote_node_ptr, this->node_ptr};
        size_t attempts = 20;
        ...
        for (size_t i = 0; i < attempts; ++i) {
            ...
            for (auto node : node_array) {
                ...
                if (!is_success) {
                    ret = rcl_wait_set_clear(wait_set_ptr);
                    ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
                    ret = rcl_wait_set_add_guard_condition(wait_set_ptr, rcl_node_get_graph_guard_condition(node), NULL);
                    ASSERT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
                    std::chrono::nanoseconds time_to_sleep = std::chrono::milliseconds(200);
                    RCUTILS_LOG_INFO_NAMED(ROS_PACKAGE_NAME,
                                           "  state wrong, waiting up to '%s' nanoseconds for graph changes... ",
...
TEST_F(NodeGraphMultiNodeFixture, test_node_info_subscriptions) {
    ...
    ret = rcl_subscription_init(&sub2, this->remote_node_ptr, ts, this->topic_name.c_str(), &sub_ops2);
    EXPECT_EQ(RCL_RET_OK, ret) << rcl_get_error_string().str;
    rcl_reset_error();
